...
class WashTradingDetector:
    ...
    def _check_pl_volume_ratio(self, trades):
        ...
        return {
            "value": round(ratio, 4),
            "flagged": ratio < self.thresholds["min_pl_volume_ratio"]
        }
    ...
    def _check_win_rate(self, trades):
        ...
        return {
            "value": round(rate, 2),
            "flagged": rate > self.thresholds["win_rate_suspicious"]
        }
    ...
    def _check_round_trip_patterns(self, trades):
        ...
        return {
            "count": matches,
            "flagged": matches > len(trades) * 0.3
        }
    ...
    def _check_temporal_entropy(self, trades):
        ...
        if not intervals: return {"value": 0, "flagged": False}
        ...
        entropy = -sum((c/len(intervals)) * math.log2(c/len(intervals)) for c in counts.values())
        return {
    ...
    def _check_price_impact_reversal(self, trades):
        ...
        for i in range(2, len(trades)):
            ...
            if abs(p2-p1)/p1 > 0.1 and (p3-p2)*(p2-p1) < 0: # 价格方向反转
                suspicious_moves += 1
        return {
            "value": suspicious_moves,
            "flagged": suspicious_moves > 2
        }